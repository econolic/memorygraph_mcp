...
def test_metrics_registry_tracks_counters_and_timers() -> None:
    ...
    metrics.inc("tool.kb_search.calls")
    metrics.observe("tool.kb_search.latency_ms", 12.0)
    metrics.observe("tool.kb_search.latency_ms", 18.0)
    ...
    assert counters.get("tool.kb_search.calls") == 1
    assert isinstance(p95, dict)
    assert "tool.kb_search.latency_ms" in p95
    assert "retrieval.stage.latency_ms" in p95


def test_metrics_timer_context_observes_duration() -> None:
    metrics = MetricsRegistry(prometheus_enabled=False)
    with metrics.timer("tool.kb_graph_expand.latency_ms"):
        _ = sum(range(1000))
    snap = metrics.snapshot()
    p95 = snap.get("p95_ms", {})
    assert isinstance(p95, dict)
    assert "tool.kb_graph_expand.latency_ms" in p95
