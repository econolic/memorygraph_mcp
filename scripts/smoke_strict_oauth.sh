...
python3 scripts/oidc_test_issuer.py serve \
  --host "${OIDC_HOST}" \
  --port "${OIDC_PORT}" \
  --issuer "${OIDC_ISSUER}" \
...
export KB_VECTOR_BACKEND="${KB_VECTOR_BACKEND:-memory}"
export KB_GRAPH_BACKEND="${KB_GRAPH_BACKEND:-memory}"
export KB_METADATA_BACKEND="${KB_METADATA_BACKEND:-memory}"
export KB_AUTO_INGEST_ENABLED="${KB_AUTO_INGEST_ENABLED:-false}"

PYTHONPATH=. python3 -m kb_mcp.server.transport_http >/tmp/mcp-smoke.log 2>&1 &
MCP_PID=$!
